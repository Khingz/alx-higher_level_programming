...
class TestBase_Class_Instance(unittest.TestCase):
    ...
    def test_with_several_args(self):
        ...
        """three args"""
        rect1 = Rectangle(2, 7, 4)
        rect2 = Rectangle(10, 1, 3)
        self.assertEqual(rect1.id, rect2.id - 1)

        """with four args"""
        rect1 = Rectangle(1, 2, 7, 4)
        rect2 = Rectangle(4, 10, 4, 3)
        self.assertEqual(rect1.id, rect2.id - 1)

        """with five args"""
        rect1 = Rectangle(1, 2, 7, 4, 8)
        self.assertEqual(rect1.id, 8)

        """above five args"""
        with self.assertRaises(TypeError):
            Rectangle(1, 2, 7, 4, 8, 11)

    def test_private_attr(self):
        with self.assertRaises(AttributeError):
            print(Rectangle(4, 10, 3, 3).__width)

        with self.assertRaises(AttributeError):
            print(Rectangle(4, 10, 4, 3).__height)

        with self.assertRaises(AttributeError):
            print(Rectangle(4, 10, 5, 3).__x)

        with self.assertRaises(AttributeError):
            print(Rectangle(4, 10, 3, 3).__y)

    def test_rectangle_is_base(self):
        self.assertIsInstance(Rectangle(4, 10, 3, 3), Base)

    def test_rectangle_setters(self):
        rect1 = Rectangle(1, 2, 7, 4, 8)
        rect1.width = 3
        self.assertEqual(3, rect1.width)
        rect1.height = 10
        self.assertEqual(10, rect1.height)
        rect1.x = 3
        self.assertEqual(3, rect1.x)
        rect1.y = 3
        self.assertEqual(3, rect1.y)
